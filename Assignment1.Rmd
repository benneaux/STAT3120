...
ggplot(data = posterior.dens.beta,aes(x,dens)) + 
  geom_line()
```
...
```{r Q2e}
ggplot() + 
  geom_line(data = posterior.dens.unif,aes(x,dens),color = "blue") + 
  geom_line(data = posterior.dens.beta,aes(x,dens),color = "red")
```
...
ggplot(prior.data,aes(theta.prior, prior.density)) + 
  geom_line()
```
...
ggplot(like.data,aes(theta.like, like.density)) + 
  geom_line()
```
...
ggplot() + 
  geom_line(data = coin.data,aes(theta.coin, posterior.density))
```
...
```{r Q4 BL, echo = FALSE}
a = 11; b = 0
post.treat.bl <- as.data.frame(rbeta(1000,a + 1,b + 1))
...
```{r Q$ BL 2, echo= FALSE}
a = 0; b = 1
post.control.bl <- as.data.frame(rbeta(1000,a + 1,b + 1))
...
```{r Q4 J, echo = FALSE}
a = 11; b = 0
post.treat.j <-as.data.frame(rbeta(1000,a + 0.5,b + 0.5))
...
```{r Q42, echo = FALSE}
a = 0; b = 1
post.control.j <- as.data.frame(rbeta(1000,a + 0.5,b + 0.5))
...
So, we are left with posterior distributions for both priors.

```{r Q43, echo = FALSE}
post.bl <- post.control.bl*post.treat.bl
ggplot(post.bl, aes(post.bl)) + 
  geom_histogram(binwidth = 0.1) + 
  scale_x_continuous() +
  ggtitle("Posterior Bayes-Laplace") + 
  xlab("Posterior")
```

```{r Q44, echo = FALSE}
post.j <- post.control.j*post.treat.j
ggplot(post.j, aes(post.j)) + 
  geom_histogram(binwidth = 0.1) + 
  scale_x_continuous() +
  ggtitle("Posterior Jeffrey's") + 
  xlab("Posterior")
```
...
theta.0 <- as.data.frame(seq(0,10, by = 0.001))
  ggplot2::ggplot() + 
    ...
    geom_segment(data = theta.0,x=0,xend=y,y=0,yend=0) + 
    xlim(0,10)
```
